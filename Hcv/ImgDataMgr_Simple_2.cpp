...
namespace Hcv
{
	...
	void ImgDataMgr_Simple_2::ValidateDiameter( int a_nDiameter )
	{
	...
	ImgDataMgr_Simple_2::~ImgDataMgr_Simple_2()
	{
		m_bAllowDelete = false;
	...
	void ImgDataMgr_Simple_2::PrepareData_StandDiv()
	{
		...
		ImgDataElm_Simple zeroElm;
		zeroElm.SetZeroVal();

		for(int i=0; i < nSrcSiz1D; i++)
		{
			//imgBuff[i] = m_dataArr[i].CalcMagSqr();
			imgBuff[i] = Sqr( ImgDataElm_Simple::CalcDif(
				m_dataArr[i], zeroElm ) );
		}
		...
		for(int i=0; i < nSrcSiz1D; i++)
		{
			...
			float meanSqr  = Sqr( ImgDataElm_Simple::CalcDif(
