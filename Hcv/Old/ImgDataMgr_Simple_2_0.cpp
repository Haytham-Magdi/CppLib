...
namespace Hcv
{
	...
	ImgDataMgr_Simple_2::ImgDataMgr_Simple_2(F32ImageRef a_srcImg, int a_nDiameter)
	{
	...
	void ImgDataMgr_Simple_2::PrepareAll()
	{
	...
	void ImgDataMgr_Simple_2::PrepareData_StandDiv()
	{
		CvSize srcSiz = m_srcImg->GetSize();
		const int nSrcSiz1D = srcSiz.width * srcSiz.height;


		int nAprSiz = m_nDiameter;
		//int nAprSiz = ( m_nDiameter / 2 ) + 1;
		//int nAprSiz = ( m_nDiameter / 4 ) * 2 + 1;
		...
		float nDiamSqr = nAprSiz * nAprSiz;
		...
		{
			PrepareData_First_H(nAprSiz, m_dataArr, dataArr_1);

			for(int i=0; i < nSrcSiz1D; i++)
			{
				//dataArr_1[i].DividSelfBy(nAprSiz);
			}

			PrepareData_First_V(nAprSiz, dataArr_1, m_dataArr_Mean);

			for(int i=0; i < nSrcSiz1D; i++)
			{
				m_dataArr_Mean[i].DividSelfBy(nDiamSqr);
				//m_dataArr_Mean[i].DividSelfBy(nAprSiz);
			}
		}
		...
		m_img_StandDiv_Mag = GenFastAvgImg( m_img_StandDiv_Mag, nAprSiz);
	...
	F32ImageRef ImgDataMgr_Simple_2::GenMeanImg()
	{
		return NULL;
	...
	void ImgDataMgr_Simple_2::PrepareData_First()
	{
	...
	void ImgDataMgr_Simple_2::ZeroBorder(FixedVector<ImgDataElm_Simple> & a_dataArr,
	...
	void ImgDataMgr_Simple_2::PrepareData_First_H(int a_nAprSiz,
	...
	void ImgDataMgr_Simple_2::PrepareData_First_V(int a_nAprSiz,
			FixedVector<ImgDataElm_Simple> & a_srcArr,
			FixedVector<ImgDataElm_Simple> & a_destArr)
	{
		Hcpl::Debug::Assert( 1 == a_nAprSiz % 2 );
	...
	float ImgDataMgr_Simple_2::CalcDif(int a_nIdx_1, int a_nIdx_2)
	{
	...
	float ImgDataMgr_Simple_2::Calc_UnitDif(int a_nIdx_1, int a_nIdx_2)
	{
	...
	bool ImgDataMgr_Simple_2::VerifyEdge( int a_nIdx_R1, int a_nIdx_Med1, 
		int a_nIdx_Edge, int a_nIdx_Med2, int a_nIdx_R2 )
	{
		ImgDataElm_Simple & elm_Edge = m_dataArr[ a_nIdx_Edge ]; 

		ImgDataElm_Simple & elm_Med1 = m_dataArr[ a_nIdx_Med1 ]; 
		ImgDataElm_Simple & elm_Med2 = m_dataArr[ a_nIdx_Med2 ]; 

		ImgDataElm_Simple & elm_R1 = m_dataArr[ a_nIdx_R1 ]; 
		ImgDataElm_Simple & elm_R2 = m_dataArr[ a_nIdx_R2 ]; 

		ImgDataElm_Simple elm_Edge_M12; 
	...
	IImgPopulationRef ImgDataMgr_Simple_2::CreatePopulation()
	{
		IImgPopulationRef ret = (IImgPopulation *)(new ImgPopulation_Simple(this));
