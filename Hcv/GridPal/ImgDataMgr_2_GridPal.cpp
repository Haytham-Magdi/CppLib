...
*/
	...
	void ImgDataSrc_GridPal::ClustHisto::Init()
	{
		...
		//m_clustShareArr.SetSize( GridColorPalette::GetCore()-> rClusterArr.GetSize() );

		m_clustShareArr.SetSize( 
			GridColorPalette::GetCore()->GetElmArr().GetSize() );

		for( int i=0; i < m_clustShareArr.GetSize(); i++ )		
		{
			ShareEx & rShare = m_clustShareArr[ i ];
...
*/
			...
			{
				{
					...
					F32ColorVal & rC1 = srcBuf[ nPI ];
					GridColorPalette::GetCore()->PrepareColorRep( rC1, & rep1 );

					m_pClustHisto->IncBy( rep1 );
				}
	...
	void ImgDataMgr_2_GridPal::IncHistoFrom_V( int a_nVI )
	{
		...
		GridColorPalette::Share * shareBuf = rPixShareArrColl.GetHeadAt( a_nVI );
		int nofShares = rPixShareArrColl.GetSizeAt( a_nVI );

		for( int i=0; i < nofShares; i++ )
		{
			GridColorPalette::Share & rShare = shareBuf[ i ];
	...
	void ImgDataMgr_2_GridPal::DecHistoFrom_V( int a_nVI )
	{
		...
		for( int i=0; i < nofShares; i++ )
		{
			...
			m_pClustHisto->DecAt( rShare.ElmIdx, rShare.nSize );
		}
	}
...
*/
	...
	void ImgDataMgr_2_GridPal::PrepareExData()
	{
		m_pixShareArrColl_V.SetCapacity( 
			//m_pixEntArr.GetSize() * 10, m_pixEntArr.GetSize() );
	...
	void ImgDataMgr_2_GridPal::CleanExData()
	{
			...
			Hcpl::CommonArrCollBuff< GridColorPalette::Share > & rPixShareArrColl )
	{
		...
		GridColorPalette::Share * newBuf = rPixShareArrColl.GetHeadAt( nNewIdx );
	...
	{
		for( ShareEx * pShare = m_pFstShare;
			NULL != pShare; pShare = m_pFstShare )
		{
			DecAt( pShare->ElmIdx, pShare->nSize );
		}

	} 


	void ImgDataSrc_GridPal::ClustHisto::IncBy( GridColorPalette::ColorRep & a_rRep )
	{
		for( int i=0; i < 8; i++ )
		{
			GridColorPalette::Share & rShare = a_rRep.Shares[ i ];

			if( 0 == rShare.nSize )
				continue;

			//Hcpl_ASSERT( 0 != rShare.nSize );

			IncAt( rShare.ElmIdx, rShare.nSize );
		}
	}


	void ImgDataSrc_GridPal::ClustHisto::DecBy( GridColorPalette::ColorRep & a_rRep )
	{
		for( int i=0; i < 8; i++ )
		{
			GridColorPalette::Share & rShare = a_rRep.Shares[ i ];
			...
			DecAt( rShare.ElmIdx, rShare.nSize );
		}
	}
	...
	{
		ShareEx & rShare = m_clustShareArr[ a_nIdx ];

		int nOldNofPixes = rShare.nSize;


		rShare.nSize += a_nIncVal;

		if( 0 == nOldNofPixes && 0 != rShare.nSize )
		{
	...
	void ImgDataSrc_GridPal::ClustHisto::DecAt( int a_nIdx, int a_nDecVal )
	{
		...
		if( 5663 == rShare.ElmIdx )
			a_nIdx = a_nIdx;

		rShare.nSize -= a_nDecVal;

		//Hcpl_ASSERT( rShare.nSize >= 0 );

		//if( 0 == rShare.nSize && 0 != a_nDecVal )
		if( 0 == rShare.nSize )
		{
	...
	void ImgDataSrc_GridPal::ClustHisto::AddShareToList( ShareEx & a_rShare )
	{
		ShareEx * pShare = & a_rShare;
		if( 4095 == pShare->ElmIdx )
			pShare = pShare;
		...
		if( m_pFstShare == & a_rShare )
			m_pFstShare = m_pFstShare;
	...
	void ImgDataSrc_GridPal::ClustHisto::RemoveShareFromList( ShareEx & a_rShare )
	{
		Hcpl_ASSERT( NULL != m_pFstShare );

		ShareEx * pShare = & a_rShare;

		if( NULL == pShare->pNext && NULL == pShare->pPrev )
		{
			pShare = pShare;
		}
		...
		ShareEx * pPrev = pShare->pPrev;
		ShareEx * pNext = pShare->pNext;
