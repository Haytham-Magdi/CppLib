...
namespace Hcv
{
	...
	//RegionSegmentor30::MgrOfLinkTypes::CoreStuff m_core;

	int RegionSegmentor30::RgnInfo::s_MergeOrder = 0;

	RegionSegmentor30::RegionSegmentor30(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	void RegionSegmentor30::InitRgnInfoVect(void)
	{
...
/*		F32ChannelRef srcCh0 = m_src->GetAt(0);
	...
	inline void RegionSegmentor30::CreateLink( RgnInfo * a_pRgn, 
...
*/
	...
	void RegionSegmentor30::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor30::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor30::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor30::Segment()
	{
...
/*
							float dist = -1;
							{ 
								float gv1 = GetGValue( pLA->pRgn1 );
								float gv2 = GetGValue( pLA->pRgn2 );

								dist = ( gv1 > gv2 ) ? gv1 : gv2;
								dist *= m_difScale;
							}
*/
...
/*
								{
									F32Point & pntRgn2 = pLA->pRgn2->pos;

									//float gv2 = GetRgnGradVal( pLA->pRgn2 );
									float gv2 = *m_slideRootDifImg->GetPixAt(
										pntRgn2.x, pntRgn2.y );
									...
									//float distRoot = CalcRgnDif( 
									//	pLA->pRgn1->GetRootRgn(), pLA->pRgn2 );

									float linkDist = m_linkActionMergeQues.GetLastMinIndex();
									//linkDist *= 0.75;
									...
									if( gv2 <= 1 )
										...
										absRtio = fabs( gv2 - linkDist ) / gv2;

									const F32Point testPnt( 273, 310 );

									if( testPnt.x == pntRgn2.x && testPnt.y == pntRgn2.y )
										gv2 = gv2;

									//if( gv2 > m_linkActionMergeQues.GetLastMinIndex() )
									//if( gv2 > distRoot &&
									//if( gv2 > 1500 &&
									//if( gv2 > 1000 &&
									if( 
										...
										//gv2 > linkDist + 1 &&
										absRtio > 0.3 &&
										//distRoot < GetMaxDif() - 1 )
										linkDist < GetMaxDif() - 1 )										
									{
...
/*
						{
							...
							int nLmi = m_linkActionMergeQues.GetLastMaxIndex();
...
*/
	...
	float RegionSegmentor30::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	void RegionSegmentor30::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor30::GenSegmentedImage( bool a_bShowMeanColor )
	{
	...
	F32ImageRef RegionSegmentor30::GenSegmentedImage2( bool a_bShowMeanColor )
	{
	...
	void RegionSegmentor30::ShowValleyPath(int a_x, int a_y)
	{
	...
	void RegionSegmentor30::ShowSrcPath(int a_x, int a_y)
	{
		Hcv::S16ImageRef img0 = this->GenSegmentedImage(false);
		//Hcv::F32ImageRef img1 = this->GenSegmentedImage2(false);
	...
	void RegionSegmentor30::ShowEdgeOfConflict()
	{
...
/*
	void RegionSegmentor30::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	RegionSegmentor30::LinkAction * RegionSegmentor30::CloneLinkAction(LinkAction * a_pLA )
	{
		LinkAction * pLA = &m_linkActionVect[ m_nofLinkActions++ ];
	...
	RegionSegmentor30::LinkAction * RegionSegmentor30::ProvideLinkAction(RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		// Hthm tmp
		ThrowHcvException();
...
*/
	...
	void RegionSegmentor30::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
...
/*	
				...
				//distNew = ( distRoot * 0.5 > dstGradV ) ? distRoot : dstGradV;				
	...
	bool RegionSegmentor30::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		RegionSegmentor30::RgnConflict * pConflict = GetExistingConflict( a_pMinSizRgn, a_pMaxSizRgn);
	...
	RegionSegmentor30::RgnConflict * RegionSegmentor30::GetExistingConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		//bool bRet = false;
		...
		RegionSegmentor30::RgnConflict * pConflict = minConfList.Last();
	...
	void RegionSegmentor30::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor30::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor30::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor30::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2, RgnInfo * a_pOrgEdge )
	{
		if( NULL == a_pOrgEdge )
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor30::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor30::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.SetSize(8);
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor30::ActivateNonConfRoots()
	{
...
*/
	...
	void RegionSegmentor30::ScanImageLines()
	{
...
/////////  Diag
		...
		if( bDoDiag )
		{
			...
			{
				bgnPointVect.SetCapacity( siz1.width + siz1.height - 1 );
			...
			{
				endPointVect.SetCapacity( siz1.width + siz1.height - 1 );
...
*/
	...
	void RegionSegmentor30::FilterByLocalGrad()
	{
		for( int k=0; k < m_edgeInfoVect.GetSize(); k++ )
		{
			EdgeInfo & rEI = m_edgeInfoVect[ k ];
	...
	F32ImageRef RegionSegmentor30::GenScanImage()
	{
...
/*				if( rEI.Dir != pRgn->localGradDir && pRgn->localGradVal > 0.5 )
	...
	F32ImageRef RegionSegmentor30::GenNearEdgeImage()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor30::GenBadRootImage()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	void RegionSegmentor30::PrepareLocalGrad( )
	{
	...
	void RegionSegmentor30::FilterByNearEdgeRoots()
	{
		for( int k=0; k < m_edgeInfoVect.GetSize(); k++ )
		{
			EdgeInfo & rEI = m_edgeInfoVect[ k ];
	...
	void RegionSegmentor30::MarkNearEdgeRgns()
	{
...
*/
	...
	void RegionSegmentor30::MarkNearEdgeRgns2()
	{
		static int dxArr[] = {1, 1, 0, -1};
		static int dyArr[] = {0, 1, 1, 1};
...
*/
	...
	RegionSegmentor30::RgnInfo * RegionSegmentor30::GetDeepValleyRgn( RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		{
			F32Point p1( 105, 105 );
...
*/
	...
	bool RegionSegmentor30::GetNextValleyRgn( RgnInfo * a_pRgn, RgnInfo ** a_ppOutRgn, 
	...
	void RegionSegmentor30::ProcessLineScanResult( RgnLinkDir a_dir )
	{
...
*/
	...
	F32ImageRef RegionSegmentor30::GenRootRgnImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	F32ImageRef RegionSegmentor30::GenRgnRootDifImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	F32ImageRef RegionSegmentor30::GenBadEdgeImage()
	{
...
*/					
	...
	F32ImageRef RegionSegmentor30::GenLocalGradValImage()
	{
		//F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor30::GenLocalGradImage()
	{
...
*/
	...
	void RegionSegmentor30::AssignTraceIntersections()
	{
		static int dxArr[] = {1, 1, 0, -1};
		static int dyArr[] = {0, 1, 1, 1};
	...
	F32ImageRef RegionSegmentor30::GenMergeOrderImage()
	{
		F32ImageRef ret = this->m_src->Clone();
...
*/
	...
	void RegionSegmentor30::PrepareSrcGradImg()
	{
...
//		m_srcGrad = GenMorphGradImg( m_src, 2 );
	...
	void RegionSegmentor30::PrepareValidEdgeDirs()
	{
		const int nRadius = 4;
	...
	void RegionSegmentor30::AssignBadRoots()
	{
	...
	RegionSegmentor30::BadRootAssignMgr::BadRootAssignMgr( RegionSegmentor30 * a_pSegmentor ) :
	...
	void RegionSegmentor30::BadRootAssignMgr::Proceed()
	{
		ScanImageLines();
	...
	void RegionSegmentor30::BadRootAssignMgr::ScanImageLines()
	{
		//CvSize siz1 = m_src->GetSize();
		CvSize siz1 = m_pSegmentor->m_maxPostDiffImg->GetSize();
		//CvSize siz1 = ( (RegionSegmentor30 *) m_pSegmentor)->m_maxPostDiffImg->GetSize();
...
/////////  NeDiag
	...
	void RegionSegmentor30::BadRootAssignMgr::ProcessLineScanResult( RgnLinkDir a_dir )
	{
...
*/
	...
	void RegionSegmentor30::BadRootAssignMgr::MarkNearPeakRgns()
	{
