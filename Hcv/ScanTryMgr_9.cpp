...
namespace Hcv
{
	...
	void ScanTryMgr_9::PrepareEnv( IImgDataMgrRef a_src, int a_nDif_Diameter )
	{
		m_PI_Path_1_Arr.SetCapacity( 5000 );
		m_PI_Path_2_Arr.SetCapacity( 5000 );
		...
		//m_edgeInfoArr.SetCapacity( ( srcSiz.width * srcSiz.height )  );
		m_edgeInfoArr.SetCapacity( ( srcSiz.width * srcSiz.height ) * 2  );
		...
		m_stDevLocData = new SimpleFilt1D2DataMgr< float >( m_nMaxScanSize );
	...
	void ScanTryMgr_9::FillLineData( Ns_ImgGradMgr::ScanLineItrRef a_sli )
	{
		a_sli->GotoFirst();
		...
		float * stDevLocBuf = m_stDevLocData->GetData();		
		...
		float * stDevImg_Buf = m_src->GetStandDiv_Mag_Image()->GetPixAt( 0, 0 );
		...
		do
		{
			...
			{	
				...
				float stDev_Val = stDevImg_Buf[ nCPI ];
				...
				stDevLocBuf[ nDataCnt ] = stDev_Val;
		...
		m_stDevLocData->SetSize( nDataCnt );
...
//uuuuuuuuuuuuuuuuuu
	...
	void ScanTryMgr_9::Show_Edges_Roots( )
	{
		float * angDifBuf = m_angDifData->GetData();
...
//			)->GetPixAt(0, 0);
	...
	void ScanTryMgr_9::DspLineOnImg( int a_x, int a_y,
...
//--		F32ImageRef dspImg = m_src->Clone();
		...
		for( int i=0; i < m_PI_Path_1_Arr.GetSize(); i++ )
		{
			F32ColorVal & rC1 = dspImgBuf[ m_PI_Path_1_Arr[ i ] ];

			rC1.AssignVal( 0, 200, 0 );
		}

		for( int i=0; i < m_PI_Path_2_Arr.GetSize(); i++ )
		{
			F32ColorVal & rC1 = dspImgBuf[ m_PI_Path_2_Arr[ i ] ];
	...
	//void ScanTryMgr_9::ValidateDiameter( int a_nDiameter )
	void ScanTryMgr_9::ValidateDiameter( )
	{
		int a_nDiameter = m_nDiameter;

		m_rsDiam.SetDiamFull( a_nDiameter );
	...
	ScanTryMgr_9::ScanTryMgr_9( int a_x, int a_y )
	{
...
//--		PrepareEnv( GlobalStuff::GetLinePathImg() );
		...
		m_rsDiam.SetDiamFull( m_nDiameter );
		...
		IImgDataMgrRef dm1 = new ImgDataMgr_CovMat(			
		...
		{	
			int nofDirs = m_igm_2->GetNofDirs();

			int nDir = GlobalStuff::ScanDir;

			ScanLineItrRef sil_1 = m_igm_2->GenScanLineItr( 
				pScanDbgCore->nIOA_XY, nDir );

			EdgeScan::ScanDirMgrExtRef smExt = m_scanDirExtArr[ nDir ];
			
			const int nDirNorm = smExt->GetDirNorm( nofDirs );

			ScanLineItrRef sil_R1 = m_igm_2->GenScanLineItr( 
				pScanDbgCore->nIOA_XY, nDirNorm );

			ScanLineItrRef sil_E = m_igm_2->GenScanLineItr( 
				pScanDbgCore->nIOA_XY, nDirNorm );

			ScanLineItrRef sil_R2 = m_igm_2->GenScanLineItr( 
				pScanDbgCore->nIOA_XY, nDirNorm );

			int nPI_Bgn = pScanDbgCore->nIOA_XY;


			//ScanLineItrRef sil_1 = new ScanLineItr();
		
			const int nLen1 = GlobalStuff::AprSize1D;
			//const int nLen2 = nLen1 / 2;
			const int nLen2 = nLen1 / 3;
			//const int nLen2 = nLen1 / 4;

			int i;

			{
				m_igm_2->PrepareScanLineItr( 
					sil_E, nPI_Bgn, nDirNorm );

				for( i=0; i < nLen2; i++ )
				{
					m_PI_Path_1_Arr.PushBack( sil_E->GetCurPI() );
			...
			if( false )
			{

				{
					...
					while( true )
					{
						m_PI_Path_1_Arr.PushBack( sil_1->GetCurPI() );
			...
			if( false )
			{
				...
				if( nLen2 != i )
				{
					goto PathAround_Finished;
				}

				m_igm_2->PrepareScanLineItr( 
					sil_1, sil_1->GetCurPI(), nDir );
			...
			//int nPI_1 = 60 + srcSiz.width * 70;
			//int nPI_2 = 60 + srcSiz.width * 120;



		//	for( int i=0; i < 70; i++ )
		//	{
		//		m_PI_Path_1_Arr.PushBack( nPI_1 + i );
		//		m_PI_Path_2_Arr.PushBack( nPI_2 + i );
		//	}
...
PathAround_Finished:
		...
		EdgeScan::EdgeInfo * pEI = GetEI_Of_Pix( a_x, a_y, 
			GlobalStuff::ScanDir );
	...
	ScanTryMgr_9::ScanTryMgr_9( IImgDataMgrRef a_src, int a_nDiameter )
	{
		//m_nDiameter = GlobalStuff::AprSize1D;
...
*/
	...
	void ScanTryMgr_9::ScanSli( int a_x, int a_y,
			Ns_ImgGradMgr::ScanLineItrRef a_sli )
	{
		//PrepareAperSizes();
	...
	EdgeScan::EdgeInfo * ScanTryMgr_9::GetEI_Of_Pix( int a_x, int a_y,
		int a_nDir )
	{
		CvSize srcSiz = m_src->GetSize();
	...
	EdgeScan::EdgeInfo * ScanTryMgr_9::GetEI_Of_Pix( int a_nPI, int a_nDir )
	{
		...
		if( minFnd.FindMinIdx() >= m_edgeInfoArr.GetSize() )
			return NULL;
	...
	void ScanTryMgr_9::ScanPntDir( int a_x, int a_y,
			Ns_ImgGradMgr::ScanLineItrRef a_sli )
	{
	...
	void ScanTryMgr_9::Find_Edges_Roots( int a_x, int a_y )
	{
		ImgGradMgr_2 & igm1 = *m_imgGradMgr;
		//ImgAvgingMgr & igm1 = *m_imgGradMgr;
	...
	void ScanTryMgr_9::PostProcScanLine( Ns_ImgGradMgr::ScanLineItrRef a_sli,
...
DoesConflict_Done:
	...
	EdgeScanMgr::DataDraw * ScanTryMgr_9::AddSigDraw( ISingFilt1D2Ref a_filt, 
	...
	void ScanTryMgr_9::PrepareFiltSys()
	{
		int nShiftY_1 = 100;
		int nShiftY_2 = 180;
...
///////////////////////
	...
	void ScanTryMgr_9::PrepareFiltSys_Thesis()
	{
		int nShiftY_1 = 100;
		int nShiftY_2 = 180;
...
///////////////////////
	...
	void ScanTryMgr_9::PrepareFiltSys_Thesis_2()
	{
		int nShiftY_1 = 100;
		int nShiftY_2 = 180;
...

		{
			...
			ISingFilt1D2DataReaderRef pk_out_2 = NULL;
			{
				PeakMarkerFilt1D2_2 * filt1 = new PeakMarkerFilt1D2_2( 
					//5, scanRdr, ToRdr( m_stDevLocData ), locRdr, 
					m_imgGradMgr->GetAprSiz(), scanRdr, ToRdr( m_stDevLocData ), locRdr, 
					dif2LocRdr, angDifRdr, GetLastDraw().DataRdr, ZCPosRdr, ToRdr( m_ioaData ), 
					m_edgeInfoArr, m_pixInfoArr );
...
///////////////////////
		...
		{
			//AddSigDraw( ToRdr( m_confData ), true, 
			AddSigDraw( ToRdr( m_confData ), false, 
			//AddSigDraw( ToRdr( m_scanData ), true, 
				u8ColorVal( 0, 0, 255 ), 0, nShiftY_1 );
		}
		...
		{
			AddSigDraw( ToRdr( m_stDevLocData ), true, 
	...
	void ScanTryMgr_9::PrepareFiltSys_2()
	{
		int nShiftY_1 = 100;
		int nShiftY_2 = 180;
...
///////////////////////
	...
	void ScanTryMgr_9::PrepareAperSizes()
	{
	...
	void ScanTryMgr_9::PrepareDeepRoots()
	{
		int nDeepCnt = 0;
//		int nDeepCnt = 1;
		//int nDeepCnt = 2;
	...
	void ScanTryMgr_9::PostProcessEI_Arr_2()
	{
		PrepareDeepRoots();
...
//--		F32ColorVal * colorBuf = (F32ColorVal *) m_src->GetPixAt( 0, 0 );
		...
		FixedVector< int > silDirArr(100);
		FixedVector< bool > goNextArr(100);


		FixedVector< float > & rGradDataArr = m_imgGradMgr->GetGradDataArr();


		int nLen1 = this->m_nDiameter;
		//const int nLen2 = nLen1 / 2;
		const int nLen2 = nLen1 / 3;
		//const int nLen2 = nLen1 / 4;
...
////////////////////////////////////////////////////////////////////////////////

				{
					...
					if( u_Dif_D1_D2 < ImgDataElm_CovMat::Get_FarUnitDif() &&
						u_Dif_R1_R2 < ImgDataElm_CovMat::Get_FarUnitDif() )
					{
...
////////////////////////////////////////////////////////////////////////////////
				...
				//	Path Arround Stuff
				{
					...
					float dif_E = rGradDataArr[ 
						nIOA_Edge * nofDirs + nDir ];

					float stDev_E = stDevImg_Buf[ nIOA_Edge ];
					//float stDev_E = dif_E;


					//const float stDev_E_07 = stDev_E * 0.7;
					//const float stDev_E_07 = stDev_E * 0.8;
					const float stDev_E_07 = stDev_E * 0.6;
					const float dif_E_07 = dif_E * 0.6;

					int nIOA_R1 = ioaBuf[ rEI.InrR1_IOL ];

					float stDev_R1 = stDevImg_Buf[ nIOA_R1 ];

					int nIOA_R2 = ioaBuf[ rEI.InrR2_IOL ];

					float stDev_R2 = stDevImg_Buf[ nIOA_R2 ];
					...
					silDirArr.PushBack( nDirNorm );
					goNextArr.PushBack( true );

					silDirArr.PushBack( nDirNorm );
					goNextArr.PushBack( false );

					for( int nSilIdx = 0; nSilIdx < silDirArr.GetSize(); nSilIdx++ )
					{
						int nSilDir = silDirArr[ nSilIdx ];
						bool bGoNext = goNextArr[ nSilIdx ];

						m_igm_2->PrepareScanLineItr( 
							sil_R1, nIOA_R1, nSilDir );

						m_igm_2->PrepareScanLineItr(
							sil_E, nIOA_Edge, nSilDir );

						m_igm_2->PrepareScanLineItr( 
							sil_R2, nIOA_R2, nSilDir );
						...
						for( i=0; i < nLen2; i++ )
						{
							int nIOA_R1_I = sil_R1->GetCurPI();
							float stDev_R1_I = stDevImg_Buf[ nIOA_R1_I ];

							int nIOA_E_I = sil_E->GetCurPI();
							float dif_E_I = rGradDataArr[ 
								nIOA_E_I * nofDirs + nDir ];

							float stDev_E_I = stDevImg_Buf[ nIOA_E_I ];
							//float stDev_E_I = dif_E_I;

							int nIOA_R2_I = sil_R2->GetCurPI();
							float stDev_R2_I = stDevImg_Buf[ nIOA_R2_I ];

							//maxFnd.AddValue( stDev_I );
							minFnd.AddValue( stDev_E_I );

							if( stDev_E_I < stDev_E_07 ||
								dif_E_I < dif_E_07 )
							{
								bIsCanceled = true;
								break;								
							...
							float maxRatioRE = 0.4;

							//float stDev_E_I_04 = stDev_E_I * maxRatioRE;
							float stDev_E_I_04 = stDev_E_I * 0.6;
							float dif_E_I_04 = dif_E_I * maxRatioRE;

							float min_stDev_dif_E_I_04 = 
								( stDev_E_I_04 < dif_E_I_04 ) ?
								stDev_E_I_04 : dif_E_I_04;

							if( stDev_R1_I > min_stDev_dif_E_I_04 )
							{
								bIsCanceled = true;
								break;								
							}

							if( stDev_R2_I > min_stDev_dif_E_I_04 )
							{
								bIsCanceled = true;
								break;								
							}
...
//PathAround_2_Finished:
	...
	F32ImageRef ScanTryMgr_9::GenSlopeImg( int a_nDir )
	{
		CvSize srcSiz = m_src->GetSize();
	...
	F32ImageRef ScanTryMgr_9::GenFinDifImg( int a_nDir )
	{
		CvSize srcSiz = m_src->GetSize();
	...
	F32ImageRef ScanTryMgr_9::GenMaxSlopeImg()
	{
		CvSize srcSiz = m_src->GetSize();
	...
	F32ImageRef ScanTryMgr_9::GenMinSlopeImg()
	{
		CvSize srcSiz = m_src->GetSize();
