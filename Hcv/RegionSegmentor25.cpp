...
namespace Hcv
{
	...
	//RegionSegmentor25::MgrOfLinkTypes::CoreStuff m_core;

	int RegionSegmentor25::RgnInfo::s_MergeOrder = 0;

	RegionSegmentor25::RegionSegmentor25(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	void RegionSegmentor25::InitRgnInfoVect(void)
	{
...
/*		F32ChannelRef srcCh0 = m_src->GetAt(0);
	...
	inline void RegionSegmentor25::CreateLink( RgnInfo * a_pRgn, 
...
*/
	...
	void RegionSegmentor25::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor25::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor25::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor25::Segment()
	{
...
*/
		...
		ShowImage( GenSegmentedImage2(), "GenSegmentedImage_2");	
	...
	float RegionSegmentor25::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	void RegionSegmentor25::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor25::GenSegmentedImage( bool a_bShowMeanColor )
	{
	...
	F32ImageRef RegionSegmentor25::GenSegmentedImage2( bool a_bShowMeanColor )
	{
	...
	void RegionSegmentor25::ShowEdgeOfConflict()
	{
...
/*
	void RegionSegmentor25::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	RegionSegmentor25::LinkAction * RegionSegmentor25::CloneLinkAction(LinkAction * a_pLA )
	{
		LinkAction * pLA = &m_linkActionVect[ m_nofLinkActions++ ];
	...
	RegionSegmentor25::LinkAction * RegionSegmentor25::ProvideLinkAction(RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		// Hthm tmp
		ThrowHcvException();
...
*/
	...
	void RegionSegmentor25::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
			RgnLink & a_rLink, RgnLink & a_rLink2, float a_distBef, bool a_bSetProcessed )
	{
		...
		{		
			...
			distNew = 
				//*m_srcGrad->GetPixAt( a_pRgn->pos.x, a_pRgn->pos.y ) +
				*m_srcGrad->GetPixAt( a_pRgn2->pos.x, a_pRgn2->pos.y );
	...
	bool RegionSegmentor25::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		RegionSegmentor25::RgnConflict * pConflict = GetExistingConflict( a_pMinSizRgn, a_pMaxSizRgn);
	...
	RegionSegmentor25::RgnConflict * RegionSegmentor25::GetExistingConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		//bool bRet = false;
		...
		RegionSegmentor25::RgnConflict * pConflict = minConfList.Last();
	...
	void RegionSegmentor25::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor25::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor25::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor25::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2, RgnInfo * a_pOrgEdge )
	{
		if( NULL == a_pOrgEdge )
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor25::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor25::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.SetSize(8);
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor25::ScanImageLines()
	{
...
/////////////////////////
	...
	void RegionSegmentor25::FilterByLocalGrad()
	{
		for( int k=0; k < m_edgeInfoVect.GetSize(); k++ )
		{
			EdgeInfo & rEI = m_edgeInfoVect[ k ];
	...
	F32ImageRef RegionSegmentor25::GenScanImage()
	{
...
/*				if( rEI.Dir != pRgn->localGradDir && pRgn->localGradVal > 0.5 )
	...
	F32ImageRef RegionSegmentor25::GenNearEdgeImage()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor25::GenBadRootImage()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	void RegionSegmentor25::PrepareLocalGrad( )
	{
	...
	void RegionSegmentor25::FilterByNearEdgeRoots()
	{
		for( int k=0; k < m_edgeInfoVect.GetSize(); k++ )
		{
			EdgeInfo & rEI = m_edgeInfoVect[ k ];
	...
	void RegionSegmentor25::MarkNearEdgeRgns()
	{
...
*/
...

			{
				...
				//const float thr1 = ( rEI.Value + rEI.ValueR1 ) * 0.3;
				const float thr1_1 = ( rEI.Value + rEI.ValueR1 ) * 0.1;
...

			{
				...
				//const float thr2 = ( rEI.Value + rEI.ValueR2 ) * 0.3;
				const float thr2_1 = ( rEI.Value + rEI.ValueR2 ) * 0.1;
				const float thr1_4 = ( rEI.Value + rEI.ValueR1 ) * 0.4;
	...
	void RegionSegmentor25::MarkNearEdgeRgns2()
	{
		static int dxArr[] = {1, 1, 0, -1};
		static int dyArr[] = {0, 1, 1, 1};
...
*/
			...
			{
				//const float thr1_9 = ( rEI.Value + rEI.ValueR1 ) * 0.7;
				const float thr1_9 = ( rEI.Value + rEI.ValueR1 ) * 0.9;
				//const float thr1_6 = ( rEI.Value + rEI.ValueR1 ) * 0.3;
				const float thr1_6 = ( rEI.Value + rEI.ValueR1 ) * 0.5;
				...
				while( rEI.pRgnR1 != pRgn )
				{
					...
					if( pRgn->postDif1Arr[ nDir ] < thr1_6 )
					{
						...
						//rEI.pRgnR1 = GetDeepValleyRgn( pRgn, rEI.Dir );
						RgnInfo * pRgnNew = rEI.pRgnR1 = GetDeepValleyRgn( pRgn, dirOp );
						bDone1 = true;
...
*/
			...
			{
				//const float thr2_9 = ( rEI.Value + rEI.ValueR2 ) * 0.7;
				const float thr2_9 = ( rEI.Value + rEI.ValueR2 ) * 0.9;
				//const float thr2_6 = ( rEI.Value + rEI.ValueR2 ) * 0.3;
				const float thr2_6 = ( rEI.Value + rEI.ValueR2 ) * 0.5;
				...
				while( rEI.pRgnR2 != pRgn )
				{
					...
					if( pRgn->postDif1Arr[ nDir ] < thr2_6 )
					{
						...
						RgnInfo * pRgnNew = rEI.pRgnR2 = GetDeepValleyRgn( pRgn, rEI.Dir );
						bDone2 = true;
...
*/
	...
	RegionSegmentor25::RgnInfo * RegionSegmentor25::GetDeepValleyRgn( RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
...

		{
			...
			}while( GetNextValleyRgn( pRgn, &pRgn, dir1, &dir1 ) );
...
Finish:
		if( a_pRgn != pDeepValleyRgn )
			a_pRgn->pDeepValleyRgn = pDeepValleyRgn;
	...
	bool RegionSegmentor25::GetNextValleyRgn( RgnInfo * a_pRgn, RgnInfo ** a_ppOutRgn, 
		RgnLinkDir a_dir, RgnLinkDir * a_pOutDir )
	{
		...
		for( int i=0; i < 8; i++ )
		{
			...
			float dif = *m_srcGrad->GetPixAt( a_pRgn->pos.x, a_pRgn->pos.y ) - 
	...
	void RegionSegmentor25::ProcessLineScanResult( RgnLinkDir a_dir )
	{
...
*/
	...
	F32ImageRef RegionSegmentor25::GenBadEdgeImage()
	{
...
*/					
	...
	F32ImageRef RegionSegmentor25::GenLocalGradValImage()
	{
		//F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor25::GenLocalGradImage()
	{
...
*/
	...
	void RegionSegmentor25::AssignTraceIntersections()
	{
		static int dxArr[] = {1, 1, 0, -1};
		static int dyArr[] = {0, 1, 1, 1};
	...
	F32ImageRef RegionSegmentor25::GenMergeOrderImage()
	{
		F32ImageRef ret = this->m_src->Clone();
...
*/
	...
	void RegionSegmentor25::PrepareSrcGradImg()
	{
...
//		m_srcGrad = GenMorphGradImg( m_src, 2 );
	...
	void RegionSegmentor25::PrepareValidEdgeDirs()
	{
		const int nRadius = 4;
	...
	void RegionSegmentor25::AssignBadRoots()
	{
	...
	RegionSegmentor25::BadRootAssignMgr::BadRootAssignMgr( RegionSegmentor25 * a_pSegmentor ) :
	...
	void RegionSegmentor25::BadRootAssignMgr::Proceed()
	{
		ScanImageLines();
	...
	void RegionSegmentor25::BadRootAssignMgr::ScanImageLines()
	{
		//CvSize siz1 = m_src->GetSize();
		CvSize siz1 = m_pSegmentor->m_maxPostDiffImg->GetSize();
		//CvSize siz1 = ( (RegionSegmentor25 *) m_pSegmentor)->m_maxPostDiffImg->GetSize();
...
/////////  NeDiag
	...
	void RegionSegmentor25::BadRootAssignMgr::ProcessLineScanResult( RgnLinkDir a_dir )
	{
...
*/
	...
	void RegionSegmentor25::BadRootAssignMgr::MarkNearPeakRgns()
	{
