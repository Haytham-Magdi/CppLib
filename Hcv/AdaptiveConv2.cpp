...
namespace Hcv
{
	...
	AdaptiveConv2 * AdaptiveConv2::Create(ConvKernelRef a_convKernel)
	{
		AdaptiveConv2 * ac = new AdaptiveConv2();
		ac->m_convKernel = a_convKernel;

		return ac;
	}

	S16ImageRef AdaptiveConv2::GenResult(S16ImageRef a_src, S16ImageRef a_grad)
	{
		...
		S16ChannelRef gradCh0 = a_grad->GetAt(0);
			...
			for(int x=nBgnX; x<nLimX; x++)
			{
				...
				for(int ky = 0; ky<nKerLimY; ky++)
				{
					for(int kx = 0; kx<nKerLimX; kx++)
					{
						...
						int nGrad = gradCh0->GetAt(nSrcBgnX + kx * nKerInc,
							nSrcBgnY + ky * nKerInc);
