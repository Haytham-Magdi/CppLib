...
namespace Hcv
{
	...
	//RegionSegmentor34::MgrOfLinkTypes::CoreStuff m_core;

	int RegionSegmentor34::RgnInfo::s_MergeOrder = 0;

	RegionSegmentor34::RegionSegmentor34(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	void RegionSegmentor34::InitRgnInfoVect(void)
	{
...
/*		F32ChannelRef srcCh0 = m_src->GetAt(0);
	...
	inline void RegionSegmentor34::CreateLink( RgnInfo * a_pRgn, 
...
*/
	...
	void RegionSegmentor34::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor34::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor34::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor34::Segment()
	{
		//m_pDbgRgn = & m_rgnInfoVect[ 110507 ];
		...
		for(int e=0; e < nLim_e; e++)
		{
			...
			//for(int i=0; i < 1; i++)
			{
				...
				do
				{
					...
					if( 110507 == pLA->pRgn1->nIndex )
						i = i;

					if( 110507 == pLA->pRgn2->nIndex )
						i = i;
...
*/
	...
	float RegionSegmentor34::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	void RegionSegmentor34::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor34::GenSegmentedImage( bool a_bShowMeanColor )
	{
	...
	F32ImageRef RegionSegmentor34::GenSegmentedImage2( bool a_bShowMeanColor )
	{
	...
	void RegionSegmentor34::ShowValleyPath(int a_x, int a_y)
	{
	...
	void RegionSegmentor34::ShowSrcPath(int a_x, int a_y)
	{
		//a_x = 266;
		//a_y = 426;
	...
	void RegionSegmentor34::ShowEdgeOfConflict()
	{
...
/*
	void RegionSegmentor34::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	RegionSegmentor34::LinkAction * RegionSegmentor34::CloneLinkAction(LinkAction * a_pLA )
	{
		LinkAction * pLA = &m_linkActionVect[ m_nofLinkActions++ ];
	...
	RegionSegmentor34::LinkAction * RegionSegmentor34::ProvideLinkAction( 
		//RegionSegmentor34::RgnInfo * a_pRgn, RegionSegmentor34::RgnInfo * a_pRgn2 )
	...
	RegionSegmentor34::LinkAction * RegionSegmentor34::ProvideLinkAction(RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		// Hthm tmp
		//ThrowHcvException();
	...
	void RegionSegmentor34::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
...
*/
	...
	bool RegionSegmentor34::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		RegionSegmentor34::RgnConflict * pConflict = GetExistingConflict( a_pMinSizRgn, a_pMaxSizRgn);
	...
	RegionSegmentor34::RgnConflict * RegionSegmentor34::GetExistingConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		//bool bRet = false;
		...
		RegionSegmentor34::RgnConflict * pConflict = minConfList.Last();
	...
	void RegionSegmentor34::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor34::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor34::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor34::CreateConf_WithTrace( RgnInfo * a_pRgn1, EdgeScan::EdgeInfo * a_pEI )
	{
	...
	void RegionSegmentor34::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2, RgnInfo * a_pOrgEdge )
	{
		if( NULL == a_pOrgEdge )
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor34::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor34::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.SetSize(8);
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor34::ScanImageLines()
	{
...
//*/
	...
	F32ImageRef RegionSegmentor34::GenScanImage()
	{
...
/*				if( rEI.Dir != pRgn->localGradDir && pRgn->localGradVal > 0.5 )
	...
	F32ImageRef RegionSegmentor34::GenBadSrcImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	F32ImageRef RegionSegmentor34::GenRootRgnImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
...
*/
	...
	F32ImageRef RegionSegmentor34::GenRgnRootDifImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	void RegionSegmentor34::ResetBadSrcWork()
	{
	...
	void RegionSegmentor34::MarkRgnsFromBadSrc()
	{
		RgnInfo * pTestRgn = & m_rgnInfoVect[ m_nTestRgnIdx ];
		...
		for( int i=0; i < m_rgnOf_FstBadSrcArr.GetSize(); i++ )			
		{
			...
			if( bRootMet )
			{
				...
				for( int j=0; j < curFillRgnArr.GetSize(); j++ )
				{
					...
					if( pRgn3->pShortestEI->RootSpaceDist <
...
//*/
	...
	F32ImageRef RegionSegmentor34::Gen_FalseMergeImage_2()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor34::Gen_FalseMergeImage()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor34::GenMergeOrderImage()
	{
		F32ImageRef ret = this->m_src->Clone();
...
*/
	...
	void RegionSegmentor34::PrepareSrcGradImg()
	{
