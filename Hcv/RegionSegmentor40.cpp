...
namespace Hcv
{
	...
	//RegionSegmentor40::MgrOfLinkTypes::CoreStuff m_core;

	int RegionSegmentor40::RgnInfo::s_MergeOrder = 0;

	RegionSegmentor40::RegionSegmentor40(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	void RegionSegmentor40::InitRgnInfoVect(void)
	{
...
/*		F32ChannelRef srcCh0 = m_src->GetAt(0);
	...
	inline void RegionSegmentor40::CreateLink( RgnInfo * a_pRgn, 
...
*/
	...
	void RegionSegmentor40::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor40::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor40::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor40::Segment()
	{
		//m_pDbgRgn = & m_rgnInfoVect[ 110507 ];
...
//if( e < nLim_e - 1 )
	...
	float RegionSegmentor40::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
		//float dist = m_difScale * m_imgDataMgr->CalcDif(
			//a_pRgn1->nIndex, a_pRgn2->nIndex);
		
		float dist = m_difScale * ImgDataElm_Simple::CalcDif(
	...
	void RegionSegmentor40::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor40::GenSegmentedImage( bool a_bShowMeanColor )
	{
	...
	F32ImageRef RegionSegmentor40::GenSegmentedImage2( bool a_bShowMeanColor )
	{
	...
	void RegionSegmentor40::ShowValleyPath(int a_x, int a_y)
	{
	...
	void RegionSegmentor40::ShowSrcPath(int a_x, int a_y)
	{
		//a_x = 266;
		//a_y = 426;
	...
	void RegionSegmentor40::ShowEdgeOfConflict()
	{
...
/*
	void RegionSegmentor40::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	RegionSegmentor40::LinkAction * RegionSegmentor40::CloneLinkAction(LinkAction * a_pLA )
	{
		LinkAction * pLA = &m_linkActionVect[ m_nofLinkActions++ ];
	...
	RegionSegmentor40::LinkAction * RegionSegmentor40::ProvideLinkAction( 
		//RegionSegmentor40::RgnInfo * a_pRgn, RegionSegmentor40::RgnInfo * a_pRgn2 )
	...
	RegionSegmentor40::LinkAction * RegionSegmentor40::ProvideLinkAction(RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		// Hthm tmp
		//ThrowHcvException();
	...
	void RegionSegmentor40::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
...
*/
	...
	bool RegionSegmentor40::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		RegionSegmentor40::RgnConflict * pConflict = GetExistingConflict( a_pMinSizRgn, a_pMaxSizRgn);
	...
	RegionSegmentor40::RgnConflict * RegionSegmentor40::GetExistingConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		//bool bRet = false;
		...
		RegionSegmentor40::RgnConflict * pConflict = minConfList.Last();
	...
	void RegionSegmentor40::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor40::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor40::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor40::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2, RgnInfo * a_pOrgEdge, EdgeScan::EdgeInfo * a_pEI )
	{
		if( NULL == a_pOrgEdge )
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor40::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor40::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.SetSize(8);
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor40::ScanImageLines()
	{
...
/////////--------------------------------------------
	...
	F32ImageRef RegionSegmentor40::GenScanImage()
	{
...
/*				if( rEI.Dir != pRgn->localGradDir && pRgn->localGradVal > 0.5 )
	...
	F32ImageRef RegionSegmentor40::GenBadSrcImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	F32ImageRef RegionSegmentor40::GenRootRgnImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
...
*/
	...
	F32ImageRef RegionSegmentor40::GenTraceDamImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
...
*/
	...
	F32ImageRef RegionSegmentor40::GenRgnRootDifImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	void RegionSegmentor40::ResetBadSrcWork()
	{
		Hcpl_ASSERT( false );
	...
	void RegionSegmentor40::MarkRgnsFromBadSrc()
	{
		RgnInfo * pTestRgn = & m_rgnInfoVect[ m_nTestRgnIdx ];
...
//*/
	...
	F32ImageRef RegionSegmentor40::Gen_FalseMergeImage_2()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor40::Gen_FalseMergeImage()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor40::GenMergeOrderImage()
	{
		F32ImageRef ret = this->m_src->Clone();
...
*/
	...
	void RegionSegmentor40::PrepareSrcGradImg()
	{
