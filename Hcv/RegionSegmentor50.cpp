...
namespace Hcv
{
	...
	//RegionSegmentor50::MgrOfLinkTypes::CoreStuff m_core;

	int RegionSegmentor50::RgnInfo::s_MergeOrder = 0;


	RegionSegmentor50::~RegionSegmentor50()
	{
	...
	RegionSegmentor50::RegionSegmentor50(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	void RegionSegmentor50::InitRgnInfoVect(void)
	{
...
/*		F32ChannelRef srcCh0 = m_src->GetAt(0);
	...
	inline void RegionSegmentor50::CreateLink( RgnInfo * a_pRgn, 
	...
	void RegionSegmentor50::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor50::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor50::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor50::Segment()
	{
		//m_pDbgRgn = & m_rgnInfoVect[ 110507 ];
...
//--					if( 1 == i )
	...
	float RegionSegmentor50::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
		ThrowHcvException();
	...
	void RegionSegmentor50::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor50::GenSegmentedImage( bool a_bShowMeanColor )
	{
		//Gen_SegData();
	...
	F32ImageRef RegionSegmentor50::GenSegmentedImage2( bool a_bShowMeanColor )
	{
	...
	void RegionSegmentor50::ShowValleyPath(int a_x, int a_y)
	{
	...
	void RegionSegmentor50::ShowSrcPath(int a_x, int a_y)
	{
		//a_x = 266;
		//a_y = 426;
	...
	void RegionSegmentor50::ShowEdgeOfConflict()
	{
...
/*
	void RegionSegmentor50::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	//RegionSegmentor50::LinkAction * RegionSegmentor50::CloneLinkAction(LinkAction * a_pLA )
	//{
	...
	RegionSegmentor50::LinkAction * RegionSegmentor50::ProvideLinkAction( 
		//RegionSegmentor50::RgnInfo * a_pRgn, RegionSegmentor50::RgnInfo * a_pRgn2 )
	...
	RegionSegmentor50::LinkAction * RegionSegmentor50::ProvideLinkAction(RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		// Hthm tmp
		//ThrowHcvException();
	...
	void RegionSegmentor50::PrepareLinkAction( 
...
*/
	...
	void RegionSegmentor50::Push_LA_To_MergeQues( int a_nIndex, LinkAction * pLA )
	{
		pLA->bIsActive = true;
	...
	RegionSegmentor50::LinkAction * RegionSegmentor50::PopMinPtr_From_MergeQues()
	{
		PopPtr:
	...
	RegionSegmentor50::LinkAction * RegionSegmentor50::PopMaxPtr_From_MergeQues()
	{
		PopPtr:
	...
	bool RegionSegmentor50::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		RegionSegmentor50::RgnConflict * pConflict = GetExistingConflict( a_pMinSizRgn, a_pMaxSizRgn);
	...
	RegionSegmentor50::RgnConflict * RegionSegmentor50::GetExistingConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		//bool bRet = false;
		...
		RegionSegmentor50::RgnConflict * pConflict = minConfList.Last();
	...
	void RegionSegmentor50::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor50::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor50::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor50::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2, RgnInfo * a_pOrgEdge, ImgScanMgr_Ns::EdgeInfo * a_pEI )
	{
		if( NULL == a_pOrgEdge )
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor50::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor50::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.SetSize(8);
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor50::PostScanPrepare()
	{
	...
	void RegionSegmentor50::ScanImageLines()
	{
...
////////-------------------------------------------
		...
		//MultiListQueMgr< ImgScanMgr_Ns::EdgeInfo > ei_Space_Ques;
		//
...
//	//		while( true )
//	//		{
//	//			ImgScanMgr_Ns::EdgeInfo * pEI = ei_Space_Ques.PopPtrMin();
//
//	//			if( NULL == pEI )
//	//			{
//	//				break;
...
///////////--------------------------------------------
	...
	RegionSegmentor50::RgnLink * RegionSegmentor50::GetLinkBetweenRgns( int a_nIdx_1, int a_nIdx_2 )
	{
	...
	F32ImageRef RegionSegmentor50::GenScanImage()
	{
...
/*				if( rEI.Dir != pRgn->localGradDir && pRgn->localGradVal > 0.5 )
	...
	F32ImageRef RegionSegmentor50::GenRootRgnImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
...
*/
	...
	F32ImageRef RegionSegmentor50::GenMergeOrderImage()
	{
		F32ImageRef ret = this->m_src->Clone();
...
*/
	...
	void RegionSegmentor50::PrepareSrcGradImg()
	{
...
//		m_srcGrad = GenMorphGradImg( m_src, 2 );
	...
	void RegionSegmentor50::Gen_SegData()
	{
