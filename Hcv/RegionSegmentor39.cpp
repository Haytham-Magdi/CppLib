...
namespace Hcv
{
	...
	//RegionSegmentor39::MgrOfLinkTypes::CoreStuff m_core;

	int RegionSegmentor39::RgnInfo::s_MergeOrder = 0;

	RegionSegmentor39::RegionSegmentor39(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	void RegionSegmentor39::InitRgnInfoVect(void)
	{
...
/*		F32ChannelRef srcCh0 = m_src->GetAt(0);
	...
	inline void RegionSegmentor39::CreateLink( RgnInfo * a_pRgn, 
...
*/
	...
	void RegionSegmentor39::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor39::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor39::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor39::Segment()
	{
		//m_pDbgRgn = & m_rgnInfoVect[ 110507 ];
	...
	float RegionSegmentor39::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	void RegionSegmentor39::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor39::GenSegmentedImage( bool a_bShowMeanColor )
	{
	...
	F32ImageRef RegionSegmentor39::GenSegmentedImage2( bool a_bShowMeanColor )
	{
	...
	void RegionSegmentor39::ShowValleyPath(int a_x, int a_y)
	{
	...
	void RegionSegmentor39::ShowSrcPath(int a_x, int a_y)
	{
		//a_x = 266;
		//a_y = 426;
	...
	void RegionSegmentor39::ShowEdgeOfConflict()
	{
...
/*
	void RegionSegmentor39::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	RegionSegmentor39::LinkAction * RegionSegmentor39::CloneLinkAction(LinkAction * a_pLA )
	{
		LinkAction * pLA = &m_linkActionVect[ m_nofLinkActions++ ];
	...
	RegionSegmentor39::LinkAction * RegionSegmentor39::ProvideLinkAction( 
		//RegionSegmentor39::RgnInfo * a_pRgn, RegionSegmentor39::RgnInfo * a_pRgn2 )
	...
	RegionSegmentor39::LinkAction * RegionSegmentor39::ProvideLinkAction(RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		// Hthm tmp
		//ThrowHcvException();
	...
	void RegionSegmentor39::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
...
*/
	...
	bool RegionSegmentor39::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		RegionSegmentor39::RgnConflict * pConflict = GetExistingConflict( a_pMinSizRgn, a_pMaxSizRgn);
	...
	RegionSegmentor39::RgnConflict * RegionSegmentor39::GetExistingConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		//bool bRet = false;
		...
		RegionSegmentor39::RgnConflict * pConflict = minConfList.Last();
	...
	void RegionSegmentor39::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor39::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor39::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor39::CreateConf_WithTrace( RgnInfo * a_pRgn1, EdgeScan::EdgeInfo * a_pEI )
	{
		...
		for( int i = rEI.InrR1_IOL + 1; i < rEI.InrR2_IOL; i++ )
		{
			...
			CreateConflict( a_pRgn1, pRgn, NULL, & rEI );
		}

	}

	void RegionSegmentor39::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2, RgnInfo * a_pOrgEdge, EdgeScan::EdgeInfo * a_pEI )
	{
		if( NULL == a_pOrgEdge )
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor39::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor39::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.SetSize(8);
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor39::ScanImageLines()
	{
...
/////////////////////////
		...
		m_scanTryMgr = new ScanTryMgr_3( m_imgDataMgr );
		//m_scanTryMgr = new ScanTryMgr_3( m_src );
	...
	F32ImageRef RegionSegmentor39::GenScanImage()
	{
...
/*				if( rEI.Dir != pRgn->localGradDir && pRgn->localGradVal > 0.5 )
	...
	F32ImageRef RegionSegmentor39::GenBadSrcImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	F32ImageRef RegionSegmentor39::GenRootRgnImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
...
*/
	...
	F32ImageRef RegionSegmentor39::GenRgnRootDifImage()
	{
		F32ImageRef ret = F32Image::Create( m_src->GetSize(), 3 );
	...
	void RegionSegmentor39::ResetBadSrcWork()
	{
		Hcpl_ASSERT( false );
	...
	void RegionSegmentor39::MarkRgnsFromBadSrc()
	{
		RgnInfo * pTestRgn = & m_rgnInfoVect[ m_nTestRgnIdx ];
...
//*/
	...
	F32ImageRef RegionSegmentor39::Gen_FalseMergeImage_2()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor39::Gen_FalseMergeImage()
	{
		F32ImageRef ret = this->m_src->Clone();
	...
	F32ImageRef RegionSegmentor39::GenMergeOrderImage()
	{
		F32ImageRef ret = this->m_src->Clone();
...
*/
	...
	void RegionSegmentor39::PrepareSrcGradImg()
	{
