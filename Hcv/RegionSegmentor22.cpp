...
namespace Hcv
{
	...
	//RegionSegmentor22::MgrOfLinkTypes::CoreStuff m_core;

	RegionSegmentor22::RegionSegmentor22(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	void RegionSegmentor22::InitRgnInfoVect(void)
	{
...
/*					if(nRootImgVal > 1)
	...
	inline void RegionSegmentor22::CreateLink( RgnInfo * a_pRgn, 
...
*/
	...
	void RegionSegmentor22::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor22::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor22::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor22::Segment()
	{
		...
		for(int e=0; e < 1; e++)
		{
			...
			//for(int i=0; i < 1; i++)
			{
				...
				do
				{
					...
					//GlobalStuff::AprSize1D = pLA->nIndex;
	...
	//	ValidLA :
					...
					//if( pLA->pRgn2->bInTraceAttachDone && ! pLA->pRgn2->IsRoot() )
						//continue;

					//if( pLA->pRgn1->bInTraceAttachDone && ! pLA->pRgn2->bInTraceAttachDone )
						//continue;
...
NoTraceProblem:
	...
	float RegionSegmentor22::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	void RegionSegmentor22::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor22::GenSegmentedImage( bool a_bShowMeanColor )
	{
	...
	void RegionSegmentor22::ShowEdgeOfConflict()
	{
...
/*
	void RegionSegmentor22::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	RegionSegmentor22::LinkAction * RegionSegmentor22::CloneLinkAction(LinkAction * a_pLA )
	{
		LinkAction * pLA = &m_linkActionVect[ m_nofLinkActions++ ];
	...
	RegionSegmentor22::LinkAction * RegionSegmentor22::ProvideLinkAction(RgnInfo * a_pRgn, RgnLinkDir a_dir )
	{
		int x = a_pRgn->pos.x;
		int y = a_pRgn->pos.y;
...
*/
	...
	void RegionSegmentor22::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
...
*/
	...
	bool RegionSegmentor22::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		RegionSegmentor22::RgnConflict * pConflict = GetExistingConflict( a_pMinSizRgn, a_pMaxSizRgn);
	...
	RegionSegmentor22::RgnConflict * RegionSegmentor22::GetExistingConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		//bool bRet = false;
		...
		RegionSegmentor22::RgnConflict * pConflict = minConfList.Last();
	...
	void RegionSegmentor22::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor22::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor22::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor22::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2, RgnInfo * a_pOrgEdge )
	{
		if( NULL == a_pOrgEdge )
	...
	void RegionSegmentor22::ProcessEdgeQues()
	{
			LinkAction * pLA = NULL;
...
*/
	...
	void RegionSegmentor22::AddFwdTraceUnit(RgnInfo * a_pRgn, int a_nFwdIndex, int a_nBkdIndex)
	{
		TraceUnit tu;
	...
	void RegionSegmentor22::AddBkdTraceUnit(RgnInfo * a_pRgn, int a_nFwdIndex, int a_nBkdIndex)
	{
		TraceUnit tu;
	...
	void RegionSegmentor22::PrepareTraceListPtrs()
	{
		{
	...
	void RegionSegmentor22::TraceLinkActionRoots( LinkAction * a_pLA)
...
/*				unitColorDist = F32ColorVal::Sub(
	...
	void RegionSegmentor22::ProcessLinkActionTrace( LinkAction * a_pLA )
	{
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor22::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor22::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.SetSize(8);
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor22::ScanImageLines()
	{
...
/////////////////////////
		...
		for( int k=0; k < m_edgeInfoVect.GetSize(); k++ )
		{
			...
			if( RgnLinkDir::RB == rEI.Dir )
			{
				int dx = rEI.pRgnR2->pos.x - rEI.pRgnR1->pos.x;
				int dy = rEI.pRgnR2->pos.y - rEI.pRgnR1->pos.y;

				if( dx != dy )
					dx = dx;
			}

			if( RgnLinkDir::LB == rEI.Dir )
			{
				int dx = rEI.pRgnR2->pos.x - rEI.pRgnR1->pos.x;
				int dy = rEI.pRgnR2->pos.y - rEI.pRgnR1->pos.y;
	...
	F32ImageRef RegionSegmentor22::GenScanImage()
	{
		...
		//U8ColorVal colorEdge( 0, 180, 230 );

/*
		static U8ColorVal colorEdgeArr[ ] = 
		{
			{ 0, 255, 0 },
			{ 0, 0, 255 },
			{ 0, 255, 100 },
...
/*
			colorEdgeArr[0].AssignVal( 0, 255, 0 );
			colorEdgeArr[1].AssignVal( 0, 255, 0 );
			colorEdgeArr[2].AssignVal( 0, 255, 0 );
			colorEdgeArr[3].AssignVal( 0, 255, 0 );
*/
		}
		...
		//U8ColorVal colorRootClear( 255, 0, 0 );
		//U8ColorVal colorRootClear( 255, 0, 0 );
		U8ColorVal colorRootClear( 255, 255, 255 );

		U8ColorVal colorRootInTrace( 0, 0, 255 );
		...
		for( int k=0; k < m_edgeInfoVect.GetSize(); k++ )
		{
...

			{
				...
				if( ( rEI.pEdgeRgn->pos.x < p1.x + nMarg && rEI.pEdgeRgn->pos.x > p1.x - nMarg ) &&
					( rEI.pEdgeRgn->pos.y < p1.y + nMarg && rEI.pEdgeRgn->pos.y > p1.y - nMarg ) )
					rEI.bCanceled = rEI.bCanceled;
			}
			...
			RgnInfo * rootArr[ 1 ];
				...
				//rootArr[ nCnt++ ] = rEI.pRgnR1;
				//rootArr[ nCnt++ ] = rEI.pRgnR2;
				rootArr[ nCnt++ ] = rEI.pEdgeRgn;

			bool bContinue = false;
			for( int p=0; p < nCnt; p++ )
			{
				//RgnInfo * pEdgeRgn = rEI.pEdgeRgn;
				RgnInfo * pRootRgn = rootArr[ p ];

				//if( false == pRootRgn->bInTrace )
					//continue;


				//if( 459 <= pEdgeRgn->pos.x && 469 >= pEdgeRgn->pos.x && 
				//	429 == pEdgeRgn->pos.y )
				//	pEdgeRgn = pEdgeRgn;


				//RgnLinkDir dirMax = rEI.Dir;

				//float maxValue = rEI.Value;
...

				{
					...
					if( ( rEI.pEdgeRgn->pos.x < p1.x + nMarg && rEI.pEdgeRgn->pos.x > p1.x - nMarg ) &&
						( rEI.pEdgeRgn->pos.y < p1.y + nMarg && rEI.pEdgeRgn->pos.y > p1.y - nMarg ) && 
						RgnLinkDir::CB == rEI.pEdgeRgn->localGradDir )
						rEI.bCanceled = rEI.bCanceled;
				}
				...
				for( int u=0; u < 4; u++ )
				{
					EdgeInfo * pEdgeInfo1 = pRootRgn->edgeInfoArr[ u ];

					if( NULL == pEdgeInfo1 )
						continue;


					//if( pEdgeInfo1->Value > rEI.Value )
						//rEI.bMax = false; 

					if( rEI.pEdgeRgn->bIsValleyRoot )
					//if( rEI.pEdgeRgn->bIsValleyRoot &&  ( rEI.pRgnInrR1->bIsValleyRoot ||  
						//rEI.pRgnInrR2->bIsValleyRoot ) )
						break;

								
					if( RgnLinkDir::RC != rEI.pEdgeRgn->localGradDir )
						u = u;


					//if( false )
					//if( rEI.Dir != rEI.pEdgeRgn->localGradDir &&  rEI.pEdgeRgn->localGradVal > 0 )
					//if( rEI.Dir != rEI.pEdgeRgn->localGradDir )
					if( rEI.Dir != rEI.pEdgeRgn->localGradDir ||  rEI.pEdgeRgn->localGradVal < 0.5 )
					//if( pEdgeInfo1->Value > rEI.Value )
					//if( pEdgeInfo1->Value > rEI.Value * 2 )
					//if( pEdgeInfo1->Value > rEI.Value * 1.5 )
					//if( pEdgeInfo1->Value > rEI.Value * 1.3 )
					//if( pEdgeInfo1->Value > rEI.Value * 1.3 )
					//if( pEdgeInfo1->Value > rEI.Value * 1.1 )
					{
						bContinue = true;
						goto Continue;
					}
...
Continue:
			if( bContinue )
			//if( bContinue || 
				//rEI.pRgnR1->bInTrace || rEI.pRgnR2->bInTrace )
			{
				rEI.bCanceled = true;				
				continue;
			}
...
//				if( 125 < pRootRgn->pos.x && 204 > pRootRgn->pos.x )
//					pRootRgn = pRootRgn;

//				if( 0 == pEdgeRgn->pos.x && 100 == pEdgeRgn->pos.y )
//					pEdgeRgn = pEdgeRgn;
...
/*				if( rEI.pEdgeRgn->bIsValleyRoot && ! rEI.bCanceled )
				{
					//color1.AssignVal( 255, 255, 255 );

					DrawCircle( ret, rEI.pEdgeRgn->pos, color1, 3);
				}
				else */
				{
	...
	void RegionSegmentor22::ProcessLineScanResult( RgnLinkDir a_dir )
	{
...
*/
	...
	F32ImageRef RegionSegmentor22::GenLocalGradImage()
	{
		...
		for( int i=0; i < m_rgnInfoVect.GetSize(); i++ )
		{
			...
			F32ColorVal color1 = colorEdgeArr[ (int)pRgn->localGradDir ];

			//if( 0 == pRgn->localGradVal )
			if( 0 == pRgn->localGradValArr[ (int)pRgn->localGradDir ] )
...
*/
	...
	void RegionSegmentor22::AssignTraceIntersections()
	{
		static int dxArr[] = {1, 1, 0, -1};
		static int dyArr[] = {0, 1, 1, 1};
