...
namespace Hcv
{
	...
	CircleContourOfPix4::CircleContourOfPix4( F32ImageRef a_srcImg, F32ImageRef a_gradImg, int a_radius )		
	{
...
*/
	...
	void CircleContourOfPix4::InitDirArr()
	{
	...
	void CircleContourOfPix4::PreparePix( int a_x, int a_y )
	{
		m_x = a_x;
		m_y = a_y;
...
*/
	...
	void CircleContourOfPix4::ShowResult()
	{
...
*/
	...
	void CircleContourOfPix4::ShowValArrSignal( FixedVector< float > & a_valArr, char * a_sWndName )
	{
...
*/
	...
	void CircleContourOfPix4::PrepareValArr2( )
	{
		...
		LineStepper ls1;
		{
			...
			//float cosVal = cos( centerRad );
			//float sinVal = sin( centerRad );

			F32Point difPnt;
			{
				float difX = cos( centerRad );
				float difY = sin( centerRad );

				float maxDifXY = ( fabs( difX ) > fabs( difY ) ) ? difX : difY;

				difPnt.x = difX / maxDifXY;
				difPnt.y = difY / maxDifXY;
			}
			//difPnt = m_pntArr[ nCenterIdx ];

			F32Point fwdPnt = F32Point::Add( srcPnt,  difPnt);
			F32Point bkdPnt = F32Point::Add( srcPnt, difPnt.MultBy(-1) );
