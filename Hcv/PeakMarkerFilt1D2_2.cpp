...
namespace Hcv
{
	...
	PeakMarkerFilt1D2_2::PeakMarkerFilt1D2_2( int a_nAprSizOrg, 
	...
	{	
		...
		m_stDevRdr = a_stDevRdr;
		m_inpRdrArr.PushBack( m_stDevRdr );
		...
		Init( 0, NULL, a_outDataMgr, a_dmyVal );
	}


	void PeakMarkerFilt1D2_2::SetupOutputMgr()
	{
	...
	void PeakMarkerFilt1D2_2::Proceed_0()
	{
		//Proceed_2();
	...
	//				aa = aa;
		...
		float * stDevBuf = m_stDevRdr->GetData();
...
*/
	...
	void PeakMarkerFilt1D2_2::Proceed()
	{
		int aa;
...
*/
	...
	void PeakMarkerFilt1D2_2::PreparePVPoses_02()
	{
	...
	void PeakMarkerFilt1D2_2::PreparePVPoses()
	{
		m_ZCIndexArr.ResetSize();
		...
		float * difBuf = m_difRdr->GetData();
		float * stDevLocBuf = m_stDevRdr->GetData();
		float * locBuf = m_locRdr->GetData();		
		float * dif2Buf = m_dif2Rdr->GetData();
		float * angDifBuf = m_angDifRdr->GetData();
		float * ZCDataBuf = m_ZCDataRdr->GetData();
		...
		int nMargBef = m_outDataMgr->GetMargBef();
		int nMargAft = m_outDataMgr->GetMargAft();

		//const int nMargBef_D = m_difRdr->GetMargBef();
		//const int nMargAft_D = m_difRdr->GetMargAft();

		const int nMargBef_D = m_nDifAprSiz / 2;
		const int nMargAft_D = m_nDifAprSiz / 2;


		//const int nMargBef_D = nMargBef;
		//const int nMargAft_D = nMargAft;
		...
		else
		{
...

			{
				...
				const int nLim2_D = nBufSiz - nMargAft_D;

				for( int i = nMargBef_D; i < nLim2_D; i++ )
				{
					//out_2_Buf[ i ] = 0;

					float difVal = difBuf[ i ];



					int nIdx_Bef = i - nMargBef_D;
					int nIdx_Aft = i + nMargAft_D;


					int nIOA_R1 = ioaBuf[ nIdx_Bef ];
					int nIOA_R2 = ioaBuf[ nIdx_Aft ];
					...
					float stVal_Cur = stDevLocBuf[ i ];
					if( stVal_Cur < 8 )
						continue;

					//if( stVal_Cur > ( difVal * 0.6 ) )
					//if( stVal_Cur > ( difVal * 0.8 ) )
						//continue;

					float stVal_Bef = stDevLocBuf[ nIdx_Bef ];
					float stVal_Aft = stDevLocBuf[ nIdx_Aft ];

					//float maxRatio = 0.8;
					float maxRatio = 0.6;
					//float maxRatio = 0.4;

					if( stVal_Bef > ( stVal_Cur * maxRatio ) )
						continue;

					if( stVal_Aft > ( stVal_Cur * maxRatio ) )
						continue;

					//float minStDif = 7;
					////float minStDif = 15;

					//if( stVal_Cur - stVal_Bef < minStDif ||
					//	stVal_Cur - stVal_Aft < minStDif)
					//	continue;


					//float maxRatio_Dif = 0.6;
					float maxRatio_Dif = 0.4;
					//float maxRatio_Dif = 0.25;
					

					if( stVal_Bef > ( difVal * maxRatio_Dif ) )
						continue;

					if( stVal_Aft > ( difVal * maxRatio_Dif ) )
						continue;


					int nShowVal = 50;

					m_ZCIndexArr.PushBack( nIdx_Bef );
					out_2_Buf[ nIdx_Bef ] = - nShowVal;
					//out_2_Buf[ nIdx_Bef ] = 2 * nShowVal;

					m_ZCIndexArr.PushBack( i );
					out_2_Buf[ i ] = nShowVal;

					m_ZCIndexArr.PushBack( nIdx_Aft );
					out_2_Buf[ nIdx_Aft ] = - nShowVal;
					//out_2_Buf[ nIdx_Aft ] = 2 * nShowVal;
				}
