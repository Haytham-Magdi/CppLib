...
namespace Hcv
{
	...
	IImgDataMgr_2 * ImgDataMgr_2_LocHist_Factory::CreateImgDataMgr( 
			F32ImageRef a_srcImg, int a_nAprSiz,
			IImgCoreSrcRef a_dataSrc, ImgRotationMgrRef a_rot )
	{
		IImgDataMgr_2 * pIdm = new ImgDataMgr_2_LocHist(
	...
	ImgDataSrc_LocHist::ImgDataSrc_LocHist( F32ImageRef a_srcImg )
	{
		m_srcImg = a_srcImg;
	...
	void ImgDataSrc_LocHist::Proceed()
	{
		int nAprSiz = 7;
	...
	F32ImageRef ImgDataSrc_LocHist::GenResultImg()
	{
	...
	void ImgDataSrc_LocHist::ClustHisto::Init( FixedVector< Cluster > & a_rClusterArr )
	{
...
/////----------------------------------------------------------------
	...
	ImgDataMgr_2_LocHist::ImgDataMgr_2_LocHist( 
		...
		ImgRotationMgrRef a_rot )
	{
		...
		//ImgDataMgr_2_LocHist * dataMgr_CM;
		m_pDataSrc_LH = NULL;
		{
			IImgCoreSrc * pDS = (IImgCoreSrc *) m_dataSrc;

			m_pDataSrc_LH = dynamic_cast< ImgDataSrc_LocHist * >( pDS );
	...
	void ImgDataMgr_2_LocHist::PrepareData()
	{
		//ThrowHcvException();
	...
	void ImgDataMgr_2_LocHist::PrepareExData()
	{
		m_pixShareArrColl_V.SetCapacity( 
			//m_pixEntArr.GetSize() * 10, m_pixEntArr.GetSize() );
			//m_srcImg->GetSize1D() * 20, m_srcImg->GetSize1D() );
			m_srcImg->GetSize1D() * 40, m_srcImg->GetSize1D() );

		m_pixShareArrColl_Tot.SetCapacity( 
			//m_pixEntArr.GetSize() * 30, m_pixEntArr.GetSize() );
			//m_srcImg->GetSize1D() * 40, m_srcImg->GetSize1D() );
			m_srcImg->GetSize1D() * 80, m_srcImg->GetSize1D() );
			//m_srcImg->GetSize1D() * 180, m_srcImg->GetSize1D() );
	...
	F32ImageRef ImgDataMgr_2_LocHist::GenResImg()
	{
		FixedVector< F32ColorVal > colorArr;
		...
		for( int i=0; i < m_pixEntArr.GetSize(); i++ )
		{
			ImgDataSrc_LocHist::PixEnt & rPE = m_pixEntArr[ i ];
	...
	void ImgDataMgr_2_LocHist::PrepareArrColl_V()
	{
			...
			{
				{
					...
					ImgDataSrc_LocHist::PixEnt & rPE = m_pixEntArr[ nPI ];
	...
	void ImgDataMgr_2_LocHist::PrepareArrColl_Tot()
	{
		CvSize imgSiz = m_srcImg->GetSize();
	...
	void ImgDataMgr_2_LocHist::IncHistoFrom_V( int a_nVI )
	{
		//ThrowHcvException();
		...
		ImgDataSrc_LocHist::ClustShare * shareBuf = rPixShareArrColl.GetHeadAt( a_nVI );
		int nofShares = rPixShareArrColl.GetSizeAt( a_nVI );

		for( int i=0; i < nofShares; i++ )
		{
			ImgDataSrc_LocHist::ClustShare & rShare = shareBuf[ i ];
	...
	void ImgDataMgr_2_LocHist::DecHistoFrom_V( int a_nVI )
	{
		//ThrowHcvException();
	...
	void ImgDataMgr_2_LocHist::PreparePixEntArr()
	{
		...
		FixedVector< ImgDataSrc_LocHist::PixEnt > 
	...
	void ImgDataMgr_2_LocHist::CleanExData()
	{
		m_pixShareArrColl_V.SetCapacity( 0, 0 );
		m_pixShareArrColl_Tot.SetCapacity( 0, 0 );
	}



	void ImgDataMgr_2_LocHist::AddToArrColl( 
			Hcpl::CommonArrCollBuff< ImgDataSrc_LocHist::ClustShare > & rPixShareArrColl )
	{
		...
		ImgDataSrc_LocHist::ClustShare * newBuf = rPixShareArrColl.GetHeadAt( nNewIdx );

		ImgDataSrc_LocHist::ClustShareEx * pShareEx = m_pClustHisto->m_pFstShare;

		int i = 0;
		for( ; NULL != pShareEx; i++ )
		{
			ImgDataSrc_LocHist::ClustShare & rShare = newBuf[ i ];
	...
	void ImgDataSrc_LocHist::ClustHisto::Reset()
	{
	...
	void ImgDataSrc_LocHist::ClustHisto::IncAt( int a_nIdx, int a_nIncVal )
	{
	...
	void ImgDataSrc_LocHist::ClustHisto::DecAt( int a_nIdx, int a_nDecVal )
	{
		ClustShareEx & rShare = m_clustShareArr[ a_nIdx ];
	...
	void ImgDataSrc_LocHist::ClustHisto::AddShareToList( ClustShareEx & a_rShare )
	{
		if( NULL == m_pFstShare )
		{
	...
	void ImgDataSrc_LocHist::ClustHisto::RemoveShareFromList( ClustShareEx & a_rShare )
	{
