...
namespace Hcv
{
	...
	RegionSegmentor3::RegionSegmentor3(S16ImageRef a_src, int a_nDifThreshold) : 
	...
	void RegionSegmentor3::InitRgnInfoVect(void)
	{
		...
		for(int y=0; y<srcCh0->GetHeight(); y++)
		{
			...
			{	
				...
				pRgnInfo->nMeanValCh0 = pRgnInfo->nValCh0 = 
					srcCh0->GetAt(x, y);
				pRgnInfo->nMeanValCh1 = pRgnInfo->nValCh1 = 
					srcCh1->GetAt(x, y);
				pRgnInfo->nMeanValCh2 = pRgnInfo->nValCh2 = 
					srcCh2->GetAt(x, y);
	...
	inline void RegionSegmentor3::CreateLink( RgnInfo * a_pRgn, 
	...
	void RegionSegmentor3::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor3::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor3::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor3::Segment()
	{
		while( m_nDifIndex < m_difQues.GetNofQues() )
		{
			...
			int nMeanDist = CalcRgnMeanDif( pActRgn1, pActRgn2 );
			...
			else
			{
				...
				pMasterRgn->nMeanValCh0 += pSlaveRgn->nMeanValCh0;
				pMasterRgn->nMeanValCh1 += pSlaveRgn->nMeanValCh1;
				pMasterRgn->nMeanValCh2 += pSlaveRgn->nMeanValCh2;
	...
	int RegionSegmentor3::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	int RegionSegmentor3::CalcRgnMeanDif(RgnInfo * a_pActRgn1, RgnInfo * a_pActRgn2)
	{
		int nDist = (int) sqrt( (double) (
			0.51 * Sqr( a_pActRgn1->nMeanValCh0 / a_pActRgn1->nPixCnt
				- a_pActRgn2->nMeanValCh0 / a_pActRgn2->nPixCnt ) +
			1.44 * Sqr( a_pActRgn1->nMeanValCh1 / a_pActRgn1->nPixCnt
				- a_pActRgn2->nMeanValCh1 / a_pActRgn2->nPixCnt ) +
			1.048 * Sqr( a_pActRgn1->nMeanValCh2 / a_pActRgn1->nPixCnt
				- a_pActRgn2->nMeanValCh2 / a_pActRgn2->nPixCnt ) ) );

		return nDist;
	}

	void RegionSegmentor3::UpdateActRgn(RgnInfo * a_pRgn)
	{
	...
	S16ImageRef RegionSegmentor3::GenSegmentedImage(void)
	{
...
/*		for(int i=0; i < m_rgnInfoVect.size(); i++)
	...
	void RegionSegmentor3::PrepareConflicts()
	{
		// To be Revised
...
//				pRgn = pRgn;
	...
	void RegionSegmentor3::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
	...
	bool RegionSegmentor3::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
	...
	void RegionSegmentor3::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor3::PrepareRgnLinkActions( RgnInfo * a_pRgn )
	{
	...
	S16ImageRef RegionSegmentor3::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
