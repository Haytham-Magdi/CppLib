...
namespace Hcv
{
	...
	//RegionSegmentor19::MgrOfLinkTypes::CoreStuff m_core;

	RegionSegmentor19::RegionSegmentor19(F32ImageRef a_src, S16ImageRef a_rootImg,
	...
	{	
		...
		m_edgeFwdRgnVect.resize(3000);
		m_edgeFwdRgnVect.resize(0);

		m_edgeBkdRgnVect.resize(3000);
		m_edgeBkdRgnVect.resize(0);
	...
	void RegionSegmentor19::InitRgnInfoVect(void)
	{
...
/*					if(nRootImgVal > 1)
	...
	inline void RegionSegmentor19::CreateLink( RgnInfo * a_pRgn, 
...
*/
	...
	void RegionSegmentor19::InitLinks(void)
	{
		...
		m_linkActionVect.resize(10000000);
		//m_linkActionVect.resize(50000000);
	...
	S16ImageRef RegionSegmentor19::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor19::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
	...
	void RegionSegmentor19::Segment()
	{
		int cnt1 = 0;
		int cnt2 = 0;
...
//		NoConflict:
	...
	float RegionSegmentor19::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	void RegionSegmentor19::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->GetActRgn() && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor19::GenSegmentedImage( bool a_bShowMeanColor )
	{
...
/*
	void RegionSegmentor19::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	*/
	...
	void RegionSegmentor19::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
	...
	bool RegionSegmentor19::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		bool bRet = false;
	...
	void RegionSegmentor19::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor19::PrepareRgnLinkActions( RgnInfo * a_pRgn, float a_distBef )
	{
...
//			if( rLink.bProcessed )
	...
	S16ImageRef RegionSegmentor19::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	void RegionSegmentor19::CreateConflict( RgnInfo * a_pRgn1, RgnInfo * a_pRgn2 )
	{
	...
	void RegionSegmentor19::ProcessEdgeQues()
	{
			LinkAction * pLA = NULL;
...
*/
	...
	void RegionSegmentor19::AddFwdTraceUnit(RgnInfo * a_pRgn, int a_nFwdIndex, int a_nBkdIndex)
	{
		TraceUnit tu;
		...
		tu.Type = 'F';
		tu.Index = m_edgeFwdRgnVect.size();
		...
		m_edgeFwdRgnVect.push_back(tu);
	}

	void RegionSegmentor19::AddBkdTraceUnit(RgnInfo * a_pRgn, int a_nFwdIndex, int a_nBkdIndex)
	{
		TraceUnit tu;
		...
		tu.Type = 'B';
		tu.Index = m_edgeBkdRgnVect.size();
		...
		m_edgeBkdRgnVect.push_back(tu);
	}

	void RegionSegmentor19::PrepareTraceListPtrs()
	{
		{
			m_edgeFwdRgnVect[0].pBkdTU = &m_edgeBkdRgnVect[0];

			for(int i=0; i < m_edgeFwdRgnVect.size()-1; i++)
			{
				m_edgeFwdRgnVect[i].pFwdTU = &m_edgeFwdRgnVect[i+1];
				m_edgeFwdRgnVect[i+1].pBkdTU = &m_edgeFwdRgnVect[i];
			}
			m_edgeFwdRgnVect.back().pFwdTU = NULL;
		}
	...
	void RegionSegmentor19::TraceLinkActionRoots( LinkAction * a_pLA)
...
/*				unitColorDist = F32ColorVal::Sub(
	...
	void RegionSegmentor19::ProcessLinkActionTrace( LinkAction * a_pLA )
	{
		if(a_pLA->bActionCanceled)
			return;

		RgnInfo * pFwdRoot = m_edgeFwdRgnVect.back().pRgn;
		RgnInfo * pBkdRoot = m_edgeBkdRgnVect.back().pRgn;
...
//	nnnnnnnnnnnnn
			...
			{
				TraceUnit * pFwdRootTU = &m_edgeFwdRgnVect.back();
				TraceUnit * pBkdRootTU = &m_edgeBkdRgnVect.back();

				TraceUnit * pFwdBgn2 = NULL;

				if(m_edgeFwdRgnVect.size() > 1)
				{
					pFwdBgn2 = &m_edgeFwdRgnVect[0];
				}
				else if(m_edgeBkdRgnVect.size() > 1)
				{
...
/*
				for(int i=m_edgeFwdRgnVect.size()-1; i >= 1; i--)
				{
				...
				for(int i=m_edgeBkdRgnVect.size()-1; i >= 1; i--)
				{
...
EndClassification:
			...
			if( a_pLA->bIntersected == false )
			{
				{
					//for(int i=0; i < m_edgeFwdRgnVect.size()-1; i++)
					for(int i=0; i < m_edgeFwdRgnVect.size(); i++)
					{
					...
					//for(int i=0; i < m_edgeBkdRgnVect.size()-1; i++)
					for(int i=0; i < m_edgeBkdRgnVect.size(); i++)
					{
			...
			for(int i=m_edgeFwdRgnVect.size()-2; i >= 0 ; i--)
			{
				RgnInfo * pFwdRgn = m_edgeFwdRgnVect[i].pRgn;
...
*/
			...
			for(int i=m_edgeBkdRgnVect.size()-2; i >= 0 ; i--)
			{
				RgnInfo * pBkdRgn = m_edgeBkdRgnVect[i].pRgn;
...
//////////////////////////////////////////////////////////////
	...
	void RegionSegmentor19::MgrOfLinkTypes::CoreStuff::InitLinkTypeMgrs()
	{
	...
	void RegionSegmentor19::MgrOfLinkTypes::CoreStuff::InitMaps()
	{
		m_linkTypeMgrVect.resize(8);
