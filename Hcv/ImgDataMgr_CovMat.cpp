...
namespace Hcv
{
	...
	void ImgDataMgr_CovMat::ValidateDiameter( int a_nDiameter )
	{
	...
	ImgDataMgr_CovMat::ImgDataMgr_CovMat(F32ImageRef a_srcImg, int a_nDiameter,
			bool a_bPrepareData_StandDiv )
	{
		m_nDel_RefCnt = 1;
		m_bAllowDelete = false;

		m_bPrepareData_StandDiv = a_bPrepareData_StandDiv;
	...
	ImgDataMgr_CovMat::~ImgDataMgr_CovMat()
	{
		m_bAllowDelete = false;
	...
	void ImgDataMgr_CovMat::PrepareAll()
	{

		PrepareData_First();

		if( m_bPrepareData_StandDiv )
		{
			PrepareData_StandDiv();
			ShowImage( this->m_img_StandDiv_Mag, "m_img_StandDiv_Mag" );
		}
	
		m_pImgPop = this->CreatePopulation();
		m_popIdx_Arr.SetSize( 2 );

		//ShowImage( this->m_img_Mag, "m_img_Mag" );
	}



	void ImgDataMgr_CovMat::PrepareData_StandDiv_0()
	{
...

		{
			...
			for(int i=0; i < nSrcSiz1D; i++)
			{
				...
				m_dataArr_Mean[i].PrepareStanDev();
			}
		}
		...
		ImgDataElm_CovMat zeroElm;
		zeroElm.SetZeroVal();

		MaxFinder< float > maxFnd0;

		for(int i=0; i < nSrcSiz1D; i++)
		{
			//imgBuff[i] = m_dataArr[i].CalcMagSqr();

			float val = ImgDataElm_CovMat::CalcDif(
				m_dataArr[i], zeroElm );

			maxFnd0.AddValue( val );
		...
		for(int i=0; i < nSrcSiz1D; i++)
		{
			...
			float meanSqr  = Sqr( ImgDataElm_CovMat::CalcDif(
	...
	void ImgDataMgr_CovMat::PrepareData_StandDiv()
	{
		CvSize srcSiz = m_srcImg->GetSize();
		const int nSrcSiz1D = srcSiz.width * srcSiz.height;
		...
		m_igm_2 = new ImgGradMgr_2( this, nDiamInr_2, nofDirs );		

		FixedVector< float > & rGradDataArr = m_igm_2->GetGradDataArr();
		...
		for( int i=0; i < nSrcSiz1D; i++ )
		{
			...
			for( int k=0; k < nofDirs; k++ )
			{
				float val = rGradDataArr[ j + k ];
	...
	F32ImageRef ImgDataMgr_CovMat::GenMeanImg()
	{
		return NULL;
	...
	void ImgDataMgr_CovMat::PrepareData_First()
	{
	...
	void ImgDataMgr_CovMat::ZeroBorder(FixedVector<ImgDataElm_CovMat> & a_dataArr,
	...
	void ImgDataMgr_CovMat::PrepareData_First_H(int a_nAprSiz,
	...
	void ImgDataMgr_CovMat::PrepareData_First_V(int a_nAprSiz,
			FixedVector<ImgDataElm_CovMat> & a_srcArr,
			FixedVector<ImgDataElm_CovMat> & a_destArr)
	{
		Hcpl::Debug::Assert( 1 == a_nAprSiz % 2 );
	...
	float ImgDataMgr_CovMat::CalcDif(int a_nIdx_1, int a_nIdx_2)
	{
	...
	float ImgDataMgr_CovMat::Calc_UnitDif(int a_nIdx_1, int a_nIdx_2)
	{
		return ImgDataElm_CovMat::Calc_UnitDif(
			m_dataArr[a_nIdx_1], m_dataArr[a_nIdx_2]);
	}


	bool ImgDataMgr_CovMat::VerifyEdge( int a_nIdx_R1, int a_nIdx_Med1, 
		int a_nIdx_Edge, int a_nIdx_Med2, int a_nIdx_R2 )
	{
		ImgDataElm_CovMat & elm_Edge = m_dataArr[ a_nIdx_Edge ]; 

		ImgDataElm_CovMat & elm_Med1 = m_dataArr[ a_nIdx_Med1 ]; 
		ImgDataElm_CovMat & elm_Med2 = m_dataArr[ a_nIdx_Med2 ]; 

		ImgDataElm_CovMat & elm_R1 = m_dataArr[ a_nIdx_R1 ]; 
		ImgDataElm_CovMat & elm_R2 = m_dataArr[ a_nIdx_R2 ]; 

		ImgDataElm_CovMat elm_Edge_M12; 
	...
	IImgPopulationRef ImgDataMgr_CovMat::CreatePopulation()
	{
		IImgPopulationRef ret = (IImgPopulation *)(new ImgPopulation_CovMat(this));
