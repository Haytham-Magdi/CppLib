...
namespace Hcv
{
	...
	void ImgDataMgr_CovMat::PrepareData_StandDiv()
	{
		...
		int nDiamCore = m_rsDiam.GetDiamCore();
		...
		float nDiamSqr = nDiamCore * nDiamCore;
		...
		{
			PrepareData_First_H( nDiamCore, m_dataArr, dataArr_1 );

			//for(int i=0; i < nSrcSiz1D; i++)
			//{
			//	//dataArr_1[i].DividSelfBy(nDiamCore);
			//}

			PrepareData_First_V( nDiamCore, dataArr_1, m_dataArr_Mean );

			for(int i=0; i < nSrcSiz1D; i++)
			{
				m_dataArr_Mean[i].DividSelfBy( nDiamSqr );
				//m_dataArr_Mean[i].DividSelfBy(nDiamCore);
			}
		}
		...
		m_img_StandDiv_Mag = GenFastAvgImg( m_img_StandDiv_Mag, nDiamCore);
