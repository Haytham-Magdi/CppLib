...
namespace Hcv
{
	...
	RegionSegmentor9::RegionSegmentor9(S16ImageRef a_src, int a_nDifThreshold) : 
...
/*		for(int cd = 1; cd < 11; cd++)
		{
			m_confCheckPtMgr.Add( cd, 0 );
			m_confCheckPtMgr.Add( cd, cd );
			m_confCheckPtMgr.Add( 0, cd );
			m_confCheckPtMgr.Add( -cd, cd );
		}*/

		{
		const int nAprSiz = 9;
		const int nAprSizHalf = nAprSiz / 2;

		for(int x = - nAprSizHalf ; x < 0; x++)
			m_confCheckPtMgr.Add( x, 0 );

		for(int y = -1; y >= - nAprSizHalf ; y-= 5)
		{
			for(int x = - nAprSizHalf; x <= nAprSizHalf; x+= 5)
			{
				if( 0 == x && 0 == y )
					continue;

				m_confCheckPtMgr.Add( x, y );
			}
		}
	...
	void RegionSegmentor9::InitRgnInfoVect(void)
	{
	...
	inline void RegionSegmentor9::CreateLink( RgnInfo * a_pRgn, 
	...
	{	
		...
		{
			rLink.bProcessed = rLinkR2.bProcessed = true;
		}
	}



	void RegionSegmentor9::InitLinks(void)
	{
	...
	S16ImageRef RegionSegmentor9::GenDifImg(bool a_bRC, bool a_bRB,
	...
	HistoGramRef RegionSegmentor9::GenDifHisto(bool a_bRC, bool a_bRB,
		bool a_bCB, bool a_bLB)
	{
...
/*			if( 0 == pActRgn2->conflictList.GetSize() && ! bFromTmp )
	...
	int RegionSegmentor9::CalcRgnDif(RgnInfo * a_pRgn1, RgnInfo * a_pRgn2)
	{
	...
	void RegionSegmentor9::UpdateActRgn(RgnInfo * a_pRgn)
	{
...
//			while(pActRgn != pActRgn->pActRgn && nofRgns < nMaxNofRgns)
	...
	S16ImageRef RegionSegmentor9::GenSegmentedImage(void)
	{
...
/*				pRgn->nMeanValCh0 /= pRgn->nPixCnt;
	...
	void RegionSegmentor9::PrepareConflicts()
	{
...
//				pRgn = pRgn;
	...
	void RegionSegmentor9::PrepareLinkAction( RgnInfo * a_pRgn, RgnInfo * a_pRgn2,
	...
	bool RegionSegmentor9::HaveConflict( RgnInfo * a_pMinSizRgn, RgnInfo * a_pMaxSizRgn)
	{
		bool bRet = false;
	...
	void RegionSegmentor9::RemoveDuplicateConflicts( RgnInfo * a_pRgn )
	{
	...
	void RegionSegmentor9::PrepareRgnLinkActions( RgnInfo * a_pRgn )
	{
	...
	S16ImageRef RegionSegmentor9::GenConflictImg(bool a_bRC, bool a_bRB,
			bool a_bCB, bool a_bLB)
	{
		S16ImageRef ret = S16Image::Create(m_src->GetSize(), 1);
		ret->SetAll(0);
	...
	bool RegionSegmentor9::CheckConflicted( RgnInfo * a_pRgn )
	{
		CvPoint orgPoint = cvPoint( a_pRgn->x, a_pRgn->y );

		for(int i=0; i < m_confCheckPtMgr.GetNofCheckPoints(); i++)
		{
			CvPoint distPoint = m_confCheckPtMgr.CalcDist( orgPoint, i );

			if( ! PointInImage( distPoint.x, distPoint.y ) )
				continue;

			RgnInfo * pRgn2 = GetPointRgn( distPoint.x, distPoint.y );
	...
	void RegionSegmentor9::PrepareRgnConflicts( RgnInfo * a_pRgn )
	{
		PrepareRgnLinkActions( a_pRgn );
